...
from Syntaxer.reverse_semantic_consts import *
from Syntaxer.reverse_print import *
from Syntaxer.reverse_stream import *
from Syntaxer.reverse_syntax_consts import *
from Syntaxer.reverse_table import Syntax_var_table
...
class Syntax:
    ...
    def __logical3(self) -> tuple[bool, str | SEMANTIC_TYPE]:
        ...
        while token[1] == TOKEN_TYPES.OP_NOT:
            ...
            token = self.__stream.get_token()
        self.__stream.unget(1)
        there_is_not: bool = amount_not % 2 != 0

        if there_is_not:
            self.__code_table.add(TOKEN_TYPES.OP_NOT)
    ...
    def __math_primary1(self) -> tuple[bool, str | SEMANTIC_TYPE]:
        ...
        while token[1] == TOKEN_TYPES.OP_MINUS:
            ...
            token = self.__stream.get_token()
        self.__stream.unget(1)
        there_is_minus: bool = amout_uminus % 2 != 0
    ...
    def __math_primary2(self) -> tuple[bool, str | SEMANTIC_TYPE]:
        ...
        if token[1] == TOKEN_TYPES.OP_POWER:
            self.__code_table.add(token[1])
            self.__code_table.add(TOKEN_TYPES.BRACKET_L) # workaround for nice negative power
            primary = self.__math_primary1()
            self.__code_table.add(TOKEN_TYPES.BRACKET_R) # workaround for nice negative power
            type = expression_type(type, primary[1], token[1])
            if not primary[0]: # syntax error absent
                print_console(
